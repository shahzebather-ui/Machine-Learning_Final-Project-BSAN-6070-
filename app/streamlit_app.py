...
# Default: Mary (Member 2) best holdout RMSE model (4 features, same exclusions as frozen DT)
MODEL_PRESETS: list[tuple[str, str, str, str]] = [
    (
        "Mary: Linear Regression (4 features, best RMSE)",
        "models/mary_linear_regression_4feat.pkl",
        "models/mary_linear_regression_4feat_metrics.json",
        "lr",
    ),
    (
        "Mary: Linear Regression (8 features, baseline)",
        "models/mary_linear_regression_8feat.pkl",
        "models/mary_linear_regression_8feat_metrics.json",
        "lr",
    ),
    (
        "Member 1: Decision Tree (frozen)",
...
def feature_columns(root: Path, metrics_rel: str) -> list[str]:
    """Use `feature_cols` from metrics JSON (Mary LR or Member 1 DT)."""
    mp = root / metrics_rel
    if mp.exists():
...
def metric_card(metrics_path: Path, model_label: str) -> None:
...
def sidebar_controls() -> dict:
    ...
    preset_labels = [p[0] for p in MODEL_PRESETS]
    model_choice = st.sidebar.selectbox(
        "Active model (default: Mary 4 feature LR)",
        preset_labels,
        index=0,
    )
...
def visuals_tab(root: Path, kind: str, metrics_path: Path) -> None:
    out_dir = root / "docs" / "analysis_outputs"

    if kind == "lr":
        st.subheader("Linear regression (Mary)")
        st.markdown(
            "<div class='custom-subtle'>Coefficients and intercept from the active metrics JSON.</div>",
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            "<div class='custom-subtle'>Images from <code>docs/analysis_outputs/</code>.</div>",
            unsafe_allow_html=True,
        )
        ...
        with c1:
            if shap_bar.exists():
                st.image(
                    str(shap_bar),
                    caption="SHAP summary (bar) — Decision Tree",
                    use_container_width=True,
                )
            else:
        ...
        with c2:
            if shap_force.exists():
                st.image(
                    str(shap_force),
                    caption="SHAP force (one row) — Decision Tree",
                    use_container_width=True,
                )
            else:
    ...
    if out_dir.exists():
        st.markdown("### Team comparison charts (optional)")
        charts = [
...
def main() -> None:
    ...
    preset_by_label = {p[0]: p for p in MODEL_PRESETS}
    _label, model_rel, metrics_rel, model_kind = preset_by_label[ui["model_choice"]]
    ...
    with tab_overview:
        ...
        metric_card(metrics_path, ui["model_choice"])

        if not model_path.exists():
            st.warning(
                f"No model at `{model_rel}` yet. After training on `{SYNTHETIC_CSV}`, "
    ...
    model = load_model(model_rel)
    cols = feature_columns(root, metrics_rel)
    with tab_manual:
        manual_prediction(model, cols)
    with tab_batch:
        csv_prediction(model, cols, ui["max_batch_rows"])
    with tab_visuals:
        visuals_tab(root, model_kind, metrics_path)
